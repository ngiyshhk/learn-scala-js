...
object AppRouter {
  ...
  val routerConfig: RouterConfig[AppPage] = RouterConfigDsl[AppPage].buildConfig { dsl =>
    import dsl._
    (emptyRule
      | staticRoute(root, LandingPage) ~> render(<.div(^.className := "markup", Header.Props(null).render, LandingView.Props(null).render))
      | dynamicRouteCT(root ~ ("#" ~ string("problem.*").option.caseClass[ProblemPage])) ~> render(<.div(^.className := "markup", Header.Props(null).render, ProblemView.Props(null).render)))
